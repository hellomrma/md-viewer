...
<html lang="ko">
  <head>
    ...
    <meta name="description" content="브라우저에서 마크다운(.md) 파일을 가독성 있게 보는 뷰어. 파일은 서버로 전송되지 않습니다." />
...
```css
:root {
  font-family: system-ui, -apple-system, "Segoe UI", "Pretendard Variable",
...
```json
{
  ...
  "include": ["vite.config.ts", "vitest.config.ts", "playwright.config.ts", "tailwind.config.ts", "postcss.config.js", "eslint.config.js"]
}
```
...
@layer components {
  .md-body { @apply leading-[1.7]; }
  .md-body h1 { @apply mt-10 mb-4 text-3xl font-semibold tracking-tight; }
  .md-body h2 { @apply mt-8 mb-3 text-2xl font-semibold tracking-tight; }
  .md-body h3 { @apply mt-6 mb-2 text-xl font-semibold; }
  .md-body h4 { @apply mt-5 mb-2 text-lg font-semibold; }
  .md-body p { @apply my-4; }
  .md-body a { @apply underline underline-offset-2 decoration-1 hover:opacity-80; }
  .md-body ul { @apply my-4 list-disc pl-6; }
  .md-body ol { @apply my-4 list-decimal pl-6; }
  .md-body li { @apply my-1; }
  .md-body blockquote { @apply my-4 border-l-4 border-ink-border dark:border-nightInk-border pl-4 text-ink-muted dark:text-nightInk-muted; }
  .md-body hr { @apply my-8 border-ink-border dark:border-nightInk-border; }
  .md-body table { @apply my-4 w-full border-collapse text-sm; }
  .md-body th, .md-body td { @apply border border-ink-border dark:border-nightInk-border px-3 py-2; }
  .md-body th { @apply bg-ink-code/60 dark:bg-nightInk-code/60 text-left font-semibold; }
  .md-body code:not(pre code) { @apply rounded bg-ink-code dark:bg-nightInk-code px-1.5 py-0.5 font-mono text-[0.9em]; }
  .md-body pre { @apply my-5 overflow-x-auto rounded-lg p-4 font-mono text-[0.9em]; }
  .md-body img { @apply my-4 max-w-full rounded; }
  .md-body .table-wrap { @apply overflow-x-auto; }

  /* Width tokens controlled by a wrapper class on the body container */
  .md-width-narrow { max-width: theme("maxWidth.prose-narrow"); }
  .md-width-normal { max-width: theme("maxWidth.prose-normal"); }
  .md-width-wide   { max-width: theme("maxWidth.prose-wide"); }

  /* Font size tokens */
  .md-font-s { font-size: 15px; }
  .md-font-m { font-size: 16px; }
  .md-font-l { font-size: 18px; }
}
...
@media print {
  ...
  .md-body { max-width: none !important; }
  .md-body pre { background: #f6f7f9 !important; color: #000 !important; }
}
```
